...
/**
 * A custom {@link feign.codec.Encoder} that supports Multipart requests. It
...
public class FeignSpringFormEncoder implements Encoder {
	...
	private final List<HttpMessageConverter<?>> converters = new RestTemplate()
	 ...
	 * @throws EncodeException
	 */
	private void encodeMultipartFormRequest(Map<String, ?> formMap,
		...
		for (Entry<String, ?> entry : formMap.entrySet()) {
			...
			if (isMultipartFile(value)) {
				map.add(entry.getKey(),
						encodeMultipartFile((MultipartFile) value));
			} else if (isMultipartFileArray(value)) {
				encodeMultipartFiles(map, entry.getKey(),
						Arrays.asList((MultipartFile[]) value));
			} else {
	...
	private boolean isMultipartFileArray(Object o) {
		return o != null
				&& o.getClass().isArray()
				&& MultipartFile.class.isAssignableFrom(o.getClass()
						.getComponentType());
	}
	...
	private void encodeRequest(Object value, HttpHeaders requestHeaders,
		...
		HttpOutputMessage dummyRequest = new HttpOutputMessageImpl(
	...
	static class MultipartFileResource extends InputStreamResource {
		...
		public MultipartFileResource(String filename, long size,
